...
class NetworkGraph(nx.DiGraph):
    ...
    def add_iface(self, node, iface_name, is_shutdown=True):
        ...
        assert iface_name not in ifaces, "%s in %s" % (iface_name, list(ifaces.keys()))
    ...
    def set_iface_addr(self, node, iface_name, addr):
        ...
        assert is_valid_add(addr)
        ifaces = self.get_ifaces(node)
        err = "Undefined iface '%s' in %s" % (iface_name, list(ifaces.keys()))
    ...
    def enable_rip(self, node, process_id=100):
        """
        Enable RIP at a given router
        :param node: local router
        :param process_id: integer
        :return: None
        """
        err = "Cannot enable RIP on router {}".format(node)
        assert self.is_local_router(node), err
        self.node[node]['rip'] = dict(process_id=process_id, networks={})
    ...
    def is_rip_enabled(self, node):
        """
        Return True if the node has RIP process
        :param node: local router
        :return: bool
        """
        if not self.is_local_router(node):
            return False
        return 'rip' in self.node[node]
    ...
    def set_edge_rip_cost(self, src, dst, cost):
        """
        Set the RIP cost of an edge
        :param src: RIP enabled local router
        :param dst: RIP enabled local router
        :param cost: int or VALUENOTSET
        :return: None
        """
        err1 = "RIP is not enabled on router: {}".format(src)
        err2 = "RIP is not enabled on router: {}".format(dst)
    ...
    def get_edge_rip_cost(self, src, dst):
        """
        Get the RIP cost of an edge
        :param src: OSPF enabled local router
        :param dst: OSPF enabled local router
        :return: None, VALUENOTSET, int
        """
        ...
        assert self.is_rip_enabled(src), err1
        assert self.is_rip_enabled(dst), err2
        return self[src][dst].get('rip_cost', None)
    ...
    def set_bgp_neighbor_iface(self, node, neighbor, iface):
        ...
        assert neighbor in neighbors, list(neighbors.keys())
